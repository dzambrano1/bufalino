...
$sql = "SELECT id, tagid, nombre, genero, raza, etapa, grupo, estatus, fecha_nacimiento, image, image2, image3, video 
        FROM bufalino" . $where_clause . " ORDER BY tagid ASC";
...
// Sexo Counts
$sexoCountsQuery = "SELECT genero, COUNT(*) as count FROM bufalino" . $where_clause . " GROUP BY genero";
...
// Raza Counts
$razaCountsQuery = "SELECT raza, COUNT(*) as count FROM bufalino" . $where_clause . " GROUP BY raza";
...
// Etapa Counts
$etapaCountsQuery = "SELECT etapa, COUNT(*) as count FROM bufalino" . $where_clause . " GROUP BY etapa";
...
// Grupos Counts
$grupoCountsQuery = "SELECT grupo, COUNT(*) as count FROM bufalino" . $where_clause . " GROUP BY grupo";
...
// Estatus Counts
$estatusCountsQuery = "SELECT estatus, COUNT(*) as count FROM bufalino" . $where_clause . " GROUP BY estatus";
...
$tagidsQuery = "SELECT tagid FROM bufalino" . $where_clause;
...
$avgPesoQuery = "
    SELECT 
        DATE_FORMAT(bh_peso_fecha, '%Y-%m') AS peso_month, 
        AVG(bh_peso_animal) AS average_weight 
    FROM 
        bh_peso 
    WHERE 
        bh_peso_tagid IN (" . str_repeat('?,', count($tagids) - 1) . "?) 
    GROUP BY 
        peso_month 
    ORDER BY 
        peso_month ASC
...
// Fetch average monthly bh_concentrado_racion * bh_concentrado_costo from bh_concentrado table
$avgRacionQuery = "
    SELECT 
        DATE_FORMAT(bh_concentrado_fecha_inicio, '%Y-%m') AS racion_month,
        AVG(bh_concentrado_racion * bh_concentrado_costo) AS average_racion_cost
    FROM 
        bh_concentrado
    GROUP BY 
        racion_month
...
<head>
    ...
    <title>Inventario Bufalino</title>
    <!-- Link to the Favicon -->
    ...
    <div class="row justify-content-center">
        <div class="col-12 col-md-11">
            <div class="row justify-content-center align-items-stretch">
                ...
                <div class="col-md-4 d-flex px-1 mb-3 mb-md-0">
                    <div class="arrow-step w-100" onclick="window.location.href='./bufalino_registros.php'" title="Ir a Registros">
...
<div class="container nav-icons-container">
    <div class="icon-button-container">
        <button onclick="window.location.href='./bufalino_register_compras.php'" class="icon-button">
    ...
    <div class="icon-button-container">
        <button onclick="window.location.href='./bufalino_register_nacimientos.php'" class="icon-button">
            <img src="./images/becerrito.png" alt="Nacimientos" class="nav-icon">
        </button>
...
window.generateAndUploadPDF = async function(tagid) {
    ...
    try {
        // Step 0: Fetch animal data to ensure we have the correct information
        const animalResponse = await fetch(`search_animal.php?query=${encodeURIComponent(tagid)}`);
        const animalData = await animalResponse.json();
        
        if (!animalData.success || !animalData.animal) {
            throw new Error('No se pudo obtener la información del animal');
        }
        
        // Update currentAnimalData with fresh data
        currentAnimalData = animalData.animal;
        
        // Step 1: Generate PDF using bufalino_report.php
        const pdfResponse = await fetch(`bufalino_report.php?tagid=${encodeURIComponent(tagid)}`, {
            headers: {
        ...
        const pdfBlobResponse = await fetch(`bufalino_report.php?download=${encodeURIComponent(filename)}`);
        ...
        console.log('PDF uploaded successfully, sourceId:', uploadData.sourceId);
...
<script>
    function deleteAnimal(button, id) {
        ...
        $.ajax({
            url: './bufalino_delete_animal.php',
            type: 'POST',
            data: { id: id },
            dataType: 'json',
    ...
    function registrar(tagid) {
        if (confirm('¿Desea generar un informe completo para este animal?')) {
            ...
            window.location.href = 'bufalino_report.php?tagid=' + encodeURIComponent(tagid);
        }
    }
</script>
<script>
    ...
    function openUpdateModal(tagid) {
        ...
        console.log('Starting AJAX request to fetch_bufalino_data.php');
        
        $.ajax({
            url: 'fetch_bufalino_data.php',
            type: 'GET',
            data: { tagid: tagid },
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" name="raza" id="updateRaza" required>
                                            <option value="">Seleccionar</option>
                                            <?php
                                            $sql_razas = "SELECT DISTINCT bc_razas_nombre FROM bc_razas ORDER BY bc_razas_nombre";
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" name="etapa" id="updateEtapa" required>
                                            <option value="">Seleccionar</option>
                                            <?php
                                            $sql_etapa = "SELECT DISTINCT etapa FROM bufalino ORDER BY etapa";
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" name="grupo" id="updateGrupo" required>
                                            <option value="">Seleccionar</option>
                                            <?php
                                            $sql_grupos = "SELECT DISTINCT bc_grupos_nombre FROM bc_grupos ORDER BY bc_grupos_nombre";
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" name="estatus" id="updateEstatus" required>
                                            <option value="">Seleccionar</option>
                                            <?php
                                            $sql_estatus = "SELECT DISTINCT bc_estatus_nombre FROM bc_estatus ORDER BY bc_estatus_nombre";
...
async function generateAnimalPDF(tagid) {
    try {
        // First, fetch the animal data to ensure we have the correct information
        const animalResponse = await fetch(`search_animal.php?query=${encodeURIComponent(tagid)}`);
        const animalData = await animalResponse.json();
        ...
        const response = await fetch(`bufalino_report.php?tagid=${encodeURIComponent(tagid)}`, {
            headers: {
                'X-Requested-With': 'XMLHttpRequest'
            }
        });
        ...
        if (!uploadData.sourceId) {
            throw new Error(uploadData.error || 'Error: No se recibió el ID de fuente del PDF');
        }
...
<style>
    ...
    /* ChatGPT-Style Modal - Updated to match bufalino style */
    #chatpdf-modal {
        position: fixed;
        top: 0;
        left: 0;
