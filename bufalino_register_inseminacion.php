...
$chartData = [];
try {
    $chartQuery = "SELECT DATE_FORMAT(bh_inseminacion_fecha, '%Y-%m') as month_year, COUNT(*) as count 
...
<title>Bufalino Register Inseminacion</title>
<!-- Link to the Favicon -->
              ...
              <tbody>
                  ...
                  try {
                      // Query to get all Female animals and their LATEST insemination record (if any)
                        $inseminacionQuery = "
                            SELECT
                                ...
                                ins.bh_inseminacion_fecha,
                                ins.bh_inseminacion_tagid, -- Will be NULL if no insemination
                                ins.bh_inseminacion_numero,
                                ins.bh_inseminacion_costo
                            FROM
                                bufalino v
                            LEFT JOIN bh_inseminacion ins
                              ON v.tagid = ins.bh_inseminacion_tagid
                            LEFT JOIN bh_inseminacion ins_check
                              ON ins.bh_inseminacion_tagid = ins_check.bh_inseminacion_tagid
                             AND ins.bh_inseminacion_fecha < ins_check.bh_inseminacion_fecha -- Check for a newer record
                            ...
                            ORDER BY
                                ...
                                ins.bh_inseminacion_fecha DESC, -- Then by latest insemination date
                      ...
                      if (empty($inseminacionData)) {
                          ...
                          try {
                              $configQuery = "SELECT b_vencimiento_inseminacion FROM b_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              
                              // Explicitly use PDO fetch method
                              $row = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row && isset($row['b_vencimiento_inseminacion'])) {
                                  $vigencia = intval($row['b_vencimiento_inseminacion']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($inseminacionData as $row) {
                              ...
                              $inseminacionFecha = $row['bh_inseminacion_fecha'] ?? null;
                              ...
                              echo "<td class='text-center'>" . ($hasInseminacion ? htmlspecialchars($row['bh_inseminacion_numero'] ?? '') : 'N/A') . "</td>";
                              echo "<td class='text-center'>" . ($hasInseminacion ? htmlspecialchars($row['bh_inseminacion_costo'] ?? '') : 'N/A') . "</td>";
