...
try {
    ...
    $salQuery = "SELECT 
                            ...
                            bh_sal_tagid
                        FROM bh_sal 
                        WHERE bh_sal_fecha_inicio IS NOT NULL 
                        AND bh_sal_fecha_fin IS NOT NULL 
                        AND bh_sal_racion IS NOT NULL 
                        AND bh_sal_costo IS NOT NULL
                        ORDER BY bh_sal_fecha_inicio ASC";
    ...
    if (empty($salData)) {
        ...
        $salQuery = "SELECT 
                                bh_sal_fecha,
                                bh_sal_fecha as bh_sal_fecha_inicio,
                                bh_sal_fecha as bh_sal_fecha_fin,
                                bh_sal_racion,
                                bh_sal_costo,
                                bh_sal_tagid
                            FROM bh_sal 
                            WHERE bh_sal_fecha IS NOT NULL 
                            AND bh_sal_racion IS NOT NULL 
                            AND bh_sal_costo IS NOT NULL
    ...
    foreach ($salData as $sal) {
        $startDate = new DateTime($sal['bh_sal_fecha_inicio']);
        $endDate = new DateTime($sal['bh_sal_fecha_fin']);
        
        // Calculate total expense value
        $totalDays = $endDate->diff($startDate)->days + 1;
        $totalValue = $sal['bh_sal_racion'] * $sal['bh_sal_costo'] * $totalDays;
        $dailyValue = $totalValue / $totalDays;
        
        error_log("Processing sal: Start=" . $sal['bh_sal_fecha_inicio'] . 
                 ", End=" . $sal['bh_sal_fecha_fin'] . 
                 ", Days=" . $totalDays . 
            ...
            try {
                      // Query to get all Animals and ALL their Sal records (if any)
                        $salQuery = "
                            ...
                            FROM
                                bufalino v
                            LEFT JOIN
                                bh_sal c ON v.tagid = c.bh_sal_tagid -- Join ALL matching sal records
                            ORDER BY
                      ...
                      if (empty($salData)) {
                          ...
                          foreach ($salData as $row) {
                              ...
                              $salFechaInicio = $row['bh_sal_fecha_inicio'] ?? null;
                              $salFechaFin = $row['bh_sal_fecha_fin'] ?? null;
                              ...
                              if ($hasSal) {
                                  ...
                                  echo '        <button class="btn btn-warning btn-sm edit-sal" 
                                                  ...
                                                  data-debug="DB: '.htmlspecialchars($row['bh_sal_producto'] ?? 'NULL').'">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="productList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $alimentos = $stmt_alimentos->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($alimentos as $alimento_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action product-option" data-value="' . htmlspecialchars($alimento_row['bc_sal_nombre']) . '">' . htmlspecialchars($alimento_row['bc_sal_nombre']) . '</button>';
                                    }
                                } catch (PDOException $e) {
