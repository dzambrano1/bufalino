...
try {
    ...
    $sql = "
        ...
        FROM bh_concentrado
        WHERE bh_concentrado_racion > 0 AND bh_concentrado_costo > 0 -- Consider only valid cost entries
        ORDER BY bh_concentrado_tagid, bh_concentrado_fecha ASC;
    ";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            $tagid = $row['bh_concentrado_tagid'];
            $fecha = $row['bh_concentrado_fecha'];
            $daily_cost = (float)$row['bh_concentrado_racion'] * (float)$row['bh_concentrado_costo'];
