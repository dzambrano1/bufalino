...
<title>Bufalino - Registros Mastitis</title>
<!-- Link to the Favicon -->
            ...
            <div class="row justify-content-center align-items-stretch">
                ...
                <div class="col-md-4 d-flex px-1 mb-3 mb-md-0">
                    <div class="arrow-step arrow-step-active w-100">
                        <span class="badge-active">🎯 Registrando Mastitis</span>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="newEntryModalLabel">
                    <i class="fas fa-plus-circle me-2"></i>Nuevo Tratamiento Mastitis
                </h5>
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT bc_mastitis_vacuna FROM bc_mastitis ORDER BY bc_mastitis_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['bc_mastitis_vacuna']) . '">' . htmlspecialchars($vacuna_row['bc_mastitis_vacuna']) . '</option>';
                                        }
              ...
              <tbody>
                  ...
                  try {
                      $mastitisQuery = "SELECT 
                                          v.id AS bufalino_id, -- Alias bufalino ID
                                          v.tagid,
                                          v.nombre,
                                          va.bh_mastitis_fecha,
                                          va.id AS mastitis_record_id, -- Alias mastitis record ID
                                          va.bh_mastitis_producto,
                                          va.bh_mastitis_dosis,
                                          va.bh_mastitis_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.bh_mastitis_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_mastitis_history
                                      FROM 
                                          bufalino v
                                      LEFT JOIN 
                                          -- Join with all mastitis records per tagid
                                          bh_mastitis va ON v.tagid = va.bh_mastitis_tagid 
                                      WHERE 
                                          v.genero = 'Hembra'
                                          AND va.bh_mastitis_tagid IS NOT NULL
                                      ORDER BY 
                                          va.bh_mastitis_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($mastitissData)) {
                          ...
                          try {
                              $configQuery = "SELECT b_vencimiento_mastitis FROM b_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['b_vencimiento_mastitis'])) {
                                  $vigencia = intval($row_config['b_vencimiento_mastitis']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($mastitissData as $row) {
                              ...
                              echo "<td>" . htmlspecialchars($row['bh_mastitis_fecha'] ?? '') . "</td>";
                              ...
                              echo "<td>" . htmlspecialchars($row['bh_mastitis_producto'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['bh_mastitis_dosis'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['bh_mastitis_costo'] ?? 'N/A') . "</td>";

                              // Calculate due date and determine status
                              try {
                                  if (!empty($row['bh_mastitis_fecha'])) {
                        ...
                        try {
                             ...
                             $noMastitisQuery = "SELECT 
                                                 ...
                                                 WHERE 
                                                     v.genero = 'Hembra'
                                                     AND v.tagid NOT IN (
                                                         SELECT DISTINCT bh_mastitis_tagid 
                                                         FROM bh_mastitis 
                                                         WHERE bh_mastitis_tagid IS NOT NULL
                                                     )
                                                 ORDER BY 
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['bc_mastitis_vacuna']) . '">' . htmlspecialchars($vacuna_row['bc_mastitis_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
