...
<title>Bufalino - Registros Brucelosis</title>
<!-- Link to the Favicon -->
            ...
            <div class="row justify-content-center align-items-stretch">
                ...
                <div class="col-md-4 d-flex px-1 mb-3 mb-md-0">
                    <div class="arrow-step arrow-step-active w-100">
                        <span class="badge-active">🎯 Registrando Brucelosis</span>
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT bc_brucelosis_vacuna FROM bc_brucelosis ORDER BY bc_brucelosis_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['bc_brucelosis_vacuna']) . '">' . htmlspecialchars($vacuna_row['bc_brucelosis_vacuna']) . '</option>';
                                        }
              ...
              <tbody>
                  ...
                  try {
                      $brucelosisQuery = "SELECT 
                                          ...
                                          v.tagid,
                                          v.nombre,
                                          va.bh_brucelosis_fecha,
                                          va.id AS brucelosis_record_id, -- Alias brucelosis record ID
                                          va.bh_brucelosis_producto,
                                          va.bh_brucelosis_dosis,
                                          va.bh_brucelosis_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.bh_brucelosis_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_brucelosis_history
                                      FROM 
                                          bufalino v
                                      LEFT JOIN 
                                          -- Join with all brucelosis records per tagid
                                          bh_brucelosis va ON v.tagid = va.bh_brucelosis_tagid 
                                      ORDER BY 
                                          va.bh_brucelosis_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($brucelosisData)) {
                          ...
                          try {
                              $configQuery = "SELECT b_vencimiento_brucelosis FROM b_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['b_vencimiento_brucelosis'])) {
                                  $vigencia = intval($row_config['b_vencimiento_brucelosis']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($brucelosisData as $row) {
                              ...
                              if ($row['in_brucelosis_history'] == 1) {
                                  ...
                                  echo "<td>" . htmlspecialchars($row['bh_brucelosis_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['bh_brucelosis_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['bh_brucelosis_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['bh_brucelosis_fecha'])) {
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['bc_brucelosis_vacuna']) . '">' . htmlspecialchars($vacuna_row['bc_brucelosis_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
