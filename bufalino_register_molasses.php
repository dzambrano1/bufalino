...
<title>Bufalino Register Molasses</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
          ...
          <div class="modal-content">
              ...
              <div class="modal-body">
                  <form id="newMolassesForm">
                      ...
                      <div class="mb-4">
                          <label for="new_molasses" class="form-label">Melaza (kg)</label>
                          <div class="input-group">
                              <span class="input-group-text"><i class="fas fa-balance-scale"></i></span>
                              <input type="number" step="0.01" class="form-control" id="new_molasses" name="molasses" required>
                          </div>
                      </div>
                      <div class="mb-4">
              ...
              <div class="modal-footer">
                  ...
                  <button type="button" class="btn btn-success" id="saveNewMolasses">
  ...
  <!-- DataTable for bh_molasses records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="bh_molassesTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
                      ...
                      <th class="text-center">Melaza (kg)</th>
              ...
              <tbody>
                  <?php
                  try {
                      // Query to get all bh_molasses records with animal name
                      $molassesQuery = "SELECT c.*, v.nombre as animal_nombre,
                                CAST((c.bh_melaza_racion * c.bh_melaza_costo) AS DECIMAL(10,2)) as total_value
                                FROM bh_melaza c 
                                LEFT JOIN bufalino v ON c.bh_melaza_tagid = v.tagid 
                                ORDER BY c.bh_melaza_fecha DESC";
                                
                      $stmt = $conn->prepare($molassesQuery);
                      $stmt->execute();
                      $molassessData = $stmt->fetchAll(PDO::FETCH_ASSOC);
                      
                      // If no data, display a message
                      if (empty($molassessData)) {
                          echo "<tr><td colspan='9' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          // Get vigencia setting for molasses records
                          ...
                          foreach ($molassessData as $row) {
                              echo "<tr>";
                              echo "<td>" . htmlspecialchars($row['id'] ?? '') . "</td>";                        
                              echo "<td>" . htmlspecialchars($row['bh_melaza_fecha'] ?? '') . "</td>";                        
                              echo "<td>" . htmlspecialchars($row['animal_nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['bh_melaza_tagid'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['bh_melaza_racion'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['bh_melaza_costo'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['total_value'] ?? '') . "</td>";
                              
                              // Calculate due date and determine status
                              try {
                                  $molassesDate = new DateTime($row['bh_melaza_fecha']);
                                  $dueDate = clone $molassesDate;
                                  ...
                                  error_log("Date error: " . $e->getMessage() . " for date: " . $row['bh_melaza_fecha']);
                              ...
                              echo '<td class="text-center">
                                  <div class="btn-group" role="group">
                                      <button class="btn btn-warning btn-sm edit-molasses" 
                                          data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                          data-tagid="' . htmlspecialchars($row['bh_melaza_tagid'] ?? '') . '"
                                          data-molasses="' . htmlspecialchars($row['bh_melaza_racion'] ?? '') . '"
                                          data-precio="' . htmlspecialchars($row['bh_melaza_costo'] ?? '') . '"
                                          data-fecha="' . htmlspecialchars($row['bh_melaza_fecha'] ?? '') . '">
                                          <i class="fas fa-edit"></i>
                                      </button>
                                      <button class="btn btn-danger btn-sm delete-molasses" 
                                          data-id="' . htmlspecialchars($row['id'] ?? '') . '">
                      ...
                      error_log("Error in molasses table: " . $e->getMessage());
...
$(document).ready(function() {
    $('#bh_molassesTable').DataTable({
        ...
        columnDefs: [
            {
                targets: [4, 5, 6], // Melaza, Precio, Valor Total columns
...
$(document).ready(function() {
    ...
    $('#saveNewMolasses').click(function() {
        // Validate the form
        var form = document.getElementById('newMolassesForm');
        ...
        Swal.fire({
            ...
            text: `¿Desea registrar el pesaje de melaza ${formData.molasses} kg para el animal con Tag ID ${formData.tagid}?`,
            icon: 'question',
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_molasses.php',
                    type: 'POST',
                    data: {
                        action: 'insert',
                        tagid: formData.tagid,
                        molasses: formData.molasses,
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            text: 'El registro de molasses ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-molasses').click(function() {
        var id = $(this).data('id');
        var tagid = $(this).data('tagid');
        var molasses = $(this).data('molasses');
        var precio = $(this).data('precio');
        var fecha = $(this).data('fecha');
        
        // Edit Molasses Modal dialog for editing

        var modalHtml = `
        <div class="modal fade" id="editMolassesModal" tabindex="-1" aria-labelledby="editMolassesModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editMolassesModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Pesaje de Melaza
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editMolassesForm">
                            ...
                            <div class="mb-4">
                                <label for="edit_molasses" class="form-label">Melaza (kg)</label>
                                <div class="input-group">
                                    <span class="input-group-text"><i class="fas fa-balance-scale"></i></span>
                                    <input type="number" step="0.01" class="form-control" id="edit_molasses" value="${molasses}" required>
                                </div>
                            </div>
                            <div class="mb-4">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-success" id="saveEditMolasses">
        ...
        $('#editMolassesModal').remove();
        
        // Add the modal to the page
        $('body').append(modalHtml);
        
        // Show the modal
        var editModal = new bootstrap.Modal(document.getElementById('editMolassesModal'));
        editModal.show();
        
        // Handle save button click
        $('#saveEditMolasses').click(function() {
            var formData = {
                id: $('#edit_id').val(),
                tagid: $('#edit_tagid').val(),
                molasses: $('#edit_molasses').val(),
    ...
    $('.delete-molasses').click(function() {
        var id = $(this).data('id');
...
<!-- Molasses Line Chart Section -->

<div class="container mt-5 mb-5">
    <div class="card shadow">
        <div class="card-header bg-success text-white">
            <h5 class="mb-0"><i class="fas fa-chart-line me-2"></i>Evolución Melaza</h5>
        </div>
        <div class="card-body">
            <div class="row mb-4">
                <div class="col-md-4">
                ...
                <div class="col-md-4">
                    <select id="dataRangeFilter" class="form-select">
                        <option value="20">Últimos 20 raciones</option>
                        <option value="50">Últimos 50 raciones</option>
                        <option value="100">Últimos 100 raciones</option>
                        <option value="all">Todas las raciones</option>
                    </select>
                </div>
            </div>
            <div class="chart-container" style="position: relative; height:50vh; width:100%">
                <canvas id="molassesChart"></canvas>
            </div>
        </div>
    </div>
</div>

<!-- Chart.js Script for Molasses Line Chart -->
<script>
$(document).ready(function() {
    let allMolassesData = [];
    let molassesChart = null;
    
    // Fetch molasses data and create the chart
    $.ajax({
        url: 'get_molasses_data.php',
        type: 'GET',
        dataType: 'json',
        success: function(data) {
            ...
            allMolassesData = data;
            populateAnimalFilter(data);
            createMolassesChart(data);
        ...
        error: function(xhr, status, error) {
            console.error('Error fetching molasses data:', error);
        }
    });
    ...
    function updateChart() {
        ...
        let filteredData = [...allMolassesData];
    ...
    function updateChartData(data) {
        if (molassesChart) {
            molassesChart.destroy();
        }
        createMolassesChart(data);
    }
    
    function createMolassesChart(data) {
        var ctx = document.getElementById('molassesChart').getContext('2d');
        ...
        var molasses = data.map(function(item) {
            return item.molasses;
        });
        
        molassesChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: labels,
                datasets: [{
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                ...
                                var tooltipText = [
                                    'Molasses: ' + context.parsed.y.toLocaleString('es-ES', {
                                        minimumFractionDigits: 2,
                                        maximumFractionDigits: 2
                                    }) + ' kg'
                                ];
                    ...
                    title: {
                        ...
                        text: function() {
                            ...
                            if (selectedAnimal !== 'all') {
                                ...
                                return 'Evolución de Melaza - ' + animalName;
                            }
                            return 'Evolución de Melaza - Todos los Animales';
                        },
                        font: {
                            size: 16
                        }
