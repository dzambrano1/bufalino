...
<title>bufalino Register Feed</title>
<!-- Link to the Favicon -->
...
<div class="container scroll-icons-container">
    <button class="btn btn-outline-secondary mb-3" type="button" onclick="window.location.href='./bufalino_register_herd.php'" class="icon-button" data-tooltip="POBLACION">
...
<div class="container text-center">
  ...
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="bh_feedTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  <?php
                  try {
                      // Query to get all bh_concentrado records with animal name
                      $feedQuery = "SELECT c.*, v.nombre as animal_nombre,
                                CAST((c.bh_concentrado_racion * c.bh_concentrado_costo) AS DECIMAL(10,2)) as total_value
                                FROM bh_concentrado c 
                                LEFT JOIN bufalino v ON c.bh_concentrado_tagid = v.tagid 
                                ORDER BY c.bh_concentrado_fecha DESC";
                      ...
                      if (empty($feedsData)) {
                          ...
                          foreach ($feedsData as $row) {
                              ...
                              echo "<td>" . htmlspecialchars($row['bh_concentrado_fecha'] ?? '') . "</td>";                        
                              echo "<td>" . htmlspecialchars($row['animal_nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['bh_concentrado_tagid'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['bh_concentrado_racion'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['bh_concentrado_costo'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['total_value'] ?? '') . "</td>";
                              
                              // Calculate due date and determine status
                              try {
                                  $feedDate = new DateTime($row['bh_concentrado_fecha']);
                                  ...
                                  error_log("Date error: " . $e->getMessage() . " for date: " . $row['bh_concentrado_fecha']);
                              ...
                              echo '<td class="text-center">
                                  <div class="btn-group" role="group">
                                      <button class="btn btn-warning btn-sm edit-feed" 
                                          ...
                                          data-tagid="' . htmlspecialchars($row['bh_concentrado_tagid'] ?? '') . '"
                                          data-feed="' . htmlspecialchars($row['bh_concentrado_racion'] ?? '') . '"
                                          data-precio="' . htmlspecialchars($row['bh_concentrado_costo'] ?? '') . '"
