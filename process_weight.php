...
if (isset($_POST['action'])) {
    ...
    try {
        ...
        if ($action === 'insert') {
            ...
            // Execute query
            $stmt = $conn->prepare("INSERT INTO bh_peso (bh_peso_tagid, bh_peso_animal, bh_peso_precio, bh_peso_fecha) VALUES (?, ?, ?, ?)");
        ...
        // UPDATE operation
            ...
            // Execute query
            $stmt = $conn->prepare("UPDATE bh_peso SET bh_peso_tagid = ?, bh_peso_animal = ?, bh_peso_precio = ?, bh_peso_fecha = ? WHERE id = ?");
        ...
        // DELETE operation
            ...
            // Execute query
            $stmt = $conn->prepare("DELETE FROM bh_peso WHERE id = ?");
            $stmt->execute([$id]);
