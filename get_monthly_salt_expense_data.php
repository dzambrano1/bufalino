...
try {
    ...
    $sql = "
        SELECT
            ...
            bh_sal_costo
        FROM bh_sal
        WHERE bh_sal_racion > 0 AND bh_sal_costo > 0 -- Consider only valid cost entries
        ORDER BY bh_sal_tagid, bh_sal_fecha ASC;
    ";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            $tagid = $row['bh_sal_tagid'];
            $fecha = $row['bh_sal_fecha'];
            $daily_cost = (float)$row['bh_sal_racion'] * (float)$row['bh_sal_costo'];
