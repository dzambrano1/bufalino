...
$chartData = [];
try {
    $chartQuery = "SELECT DATE_FORMAT(bh_parto_fecha, '%Y-%m') as month_year, COUNT(*) as count 
                     FROM bh_parto 
                     GROUP BY month_year 
...
<title>Bufalino Register Parto</title>
<!-- Link to the Favicon -->
  ...
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="partoTable" class="table table-striped table-bordered">
              ...
              <?php
                  try {
                      // Query to get all Female animals and ALL their parto records (if any)
                        $partoQuery = "
                            SELECT
                                ...
                                ins.id AS parto_id,
                                ins.bh_parto_fecha,
                                ins.bh_parto_tagid, -- Will be NULL if no parto
                                ins.bh_parto_numero
                            FROM
                                bufalino v
                            LEFT JOIN bh_parto ins
                              ON v.tagid = ins.bh_parto_tagid
                            WHERE
                                v.genero = 'Hembra'
                            ORDER BY
                                CASE WHEN ins.id IS NOT NULL THEN 0 ELSE 1 END ASC, -- Females with parto records first
                                ins.bh_parto_fecha DESC, -- Then by latest parto date
                      ...
                      if (empty($partoData)) {
                          ...
                          try {
                              $configQuery = "SELECT b_vencimiento_parto FROM b_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              
                              // Explicitly use PDO fetch method
                              $row = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row && isset($row['b_vencimiento_parto'])) {
                                  $vigencia = intval($row['b_vencimiento_parto']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($partoData as $row) {
                              ...
                              $partoFecha = $row['bh_parto_fecha'] ?? null;
                              ...
                              echo "<td class='text-center'>" . ($hasparto ? htmlspecialchars($row['bh_parto_numero'] ?? '') : 'N/A') . "</td>";                              
...
$(document).ready(function() {
    ...
    $('#saveNewParto').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Desea registrar el parto numero ${formData.numero} para el animal con Tag ID ${formData.tagid}?`,
            icon: 'question',
