...
<title>Bufalino - Registro de Vacunación Cbr</title>
<!-- Link to the Favicon -->
            ...
            <div class="row justify-content-center align-items-stretch">
                ...
                <div class="col-md-4 d-flex px-1 mb-3 mb-md-0">
                    <div class="arrow-step arrow-step-active w-100">
                        <span class="badge-active">🎯 Registrando Cbr</span>
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT bc_cbr_vacuna FROM bc_cbr ORDER BY bc_cbr_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['bc_cbr_vacuna']) . '">' . htmlspecialchars($vacuna_row['bc_cbr_vacuna']) . '</option>';
                                        }
              ...
              <tbody>
                  ...
                  try {
                      $cbrQuery = "SELECT 
                                          ...
                                          CASE WHEN va.bh_cbr_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_cbr_history
                                      FROM 
                                          bufalino v
                                      LEFT JOIN 
                                          -- Join with all cbr records per tagid
                                          bh_cbr va ON v.tagid = va.bh_cbr_tagid 
                                      ORDER BY 
                                          va.bh_cbr_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($cbrsData)) {
                          ...
                          try {
                              $configQuery = "SELECT b_vencimiento_cbr FROM b_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['b_vencimiento_cbr'])) {
                                  $vigencia = intval($row_config['b_vencimiento_cbr']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($cbrsData as $row) {
                              ...
                              echo '        <button class="btn btn-success btn-sm register-new-cbr-btn" 
                              ...
                              if ($row['in_cbr_history'] == 1) {
                                  ...
                                  echo "<td>" . htmlspecialchars($row['bh_cbr_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['bh_cbr_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['bh_cbr_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['bh_cbr_fecha'])) {
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['bc_cbr_vacuna']) . '">' . htmlspecialchars($vacuna_row['bc_cbr_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
