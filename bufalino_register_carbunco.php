...
<title>Bufalino - Registros Carbunco</title>
<!-- Link to the Favicon -->
            ...
            <div class="row justify-content-center align-items-stretch">
                ...
                <div class="col-md-4 d-flex px-1 mb-3 mb-md-0">
                    <div class="arrow-step arrow-step-active w-100">
                        <span class="badge-active">🎯 Registrando Carbunco</span>
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT bc_carbunco_vacuna FROM bc_carbunco ORDER BY bc_carbunco_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['bc_carbunco_vacuna']) . '">' . htmlspecialchars($vacuna_row['bc_carbunco_vacuna']) . '</option>';
                                        }
              ...
              <tbody>
                  ...
                  try {
                      $carbuncoQuery = "SELECT 
                                          ...
                                          v.tagid,
                                          v.nombre,
                                          va.bh_carbunco_fecha,
                                          va.id AS carbunco_record_id, -- Alias carbunco record ID
                                          va.bh_carbunco_producto,
                                          va.bh_carbunco_dosis,
                                          va.bh_carbunco_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.bh_carbunco_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_carbunco_history
                                      FROM 
                                          bufalino v
                                      LEFT JOIN 
                                          -- Join with all carbunco records per tagid
                                          bh_carbunco va ON v.tagid = va.bh_carbunco_tagid 
                                      ORDER BY 
                                          va.bh_carbunco_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($carbuncosData)) {
                          ...
                          try {
                              $configQuery = "SELECT b_vencimiento_carbunco FROM b_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['b_vencimiento_carbunco'])) {
                                  $vigencia = intval($row_config['b_vencimiento_carbunco']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($carbuncosData as $row) {
                              ...
                              echo '        <button class="btn btn-success btn-sm register-new-carbunco-btn" 
                              ...
                              echo "<td>" . htmlspecialchars($row['bh_carbunco_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_carbunco_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['bh_carbunco_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['bh_carbunco_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['bh_carbunco_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['bh_carbunco_fecha'])) {
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['bc_carbunco_vacuna']) . '">' . htmlspecialchars($vacuna_row['bc_carbunco_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
