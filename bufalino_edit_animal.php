...
if (isset($_GET['tagid']) && !empty($_GET['tagid'])) {
    ...
    try {
        ...
        error_log("Database Error in bufalino_nacimientos_get_details.php: " . $e->getMessage());
        $response = ['success' => false, 'message' => 'Error al consultar la base de datos: ' . $e->getMessage()];
    } catch (Exception $e) {
        error_log("General Error in bufalino_nacimientos_get_details.php: " . $e->getMessage());
