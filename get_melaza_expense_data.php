...
try {
    ...
    // Melaza Expense per day = bh_melaza_racion * bh_melaza_costo
    ...
    $sql = "
        SELECT 
            ...
            bh_melaza_costo
        FROM bh_melaza 
        WHERE bh_melaza_racion > 0 
          AND bh_melaza_costo > 0 
          AND bh_melaza_fecha_inicio IS NOT NULL
    ...
    if ($result) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            $startStr = $row['bh_melaza_fecha_inicio'];
            $endStr = $row['bh_melaza_fecha_fin'];
            $racion = (float)$row['bh_melaza_racion'];
            $costo = (float)$row['bh_melaza_costo'];
