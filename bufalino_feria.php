...
try {
    $base_filter_query = "FROM bufalino WHERE estatus = 'Feria' AND precio_venta > 0";
    ...
    if (empty($tagid) || $precio_venta === false || $precio_venta <= 0) {
        ...
        try {
            ...
            if (!$animal_data) {
                ...
                // Update bufalino record with precio_venta, fecha_publicacion, and estatus
                $update_sql = "UPDATE bufalino SET precio_venta = :precio_venta, fecha_publicacion = :fecha, estatus = 'Feria' WHERE tagid = :tagid";
...
$(document).ready(function() {
    ...
    window.removeFromFeria = function(tagid) {
        if (confirm('¿Está seguro que desea quitar este animal [ID: ' + tagid + '] de la feria? Esta acción cambiará su estado y quitará el precio_venta.')) {
            $.ajax({
