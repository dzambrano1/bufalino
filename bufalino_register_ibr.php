...
<title>Bufalino - Registro de Vacunación IBR</title>
<!-- Link to the Favicon -->
            ...
            <div class="row justify-content-center align-items-stretch">
                ...
                <div class="col-md-4 d-flex px-1 mb-3 mb-md-0">
                    <div class="arrow-step arrow-step-active w-100">
                        <span class="badge-active">🎯 Registrando IBR</span>
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT bc_ibr_vacuna FROM bc_ibr ORDER BY bc_ibr_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['bc_ibr_vacuna']) . '">' . htmlspecialchars($vacuna_row['bc_ibr_vacuna']) . '</option>';
                                        }
              ...
              <tbody>
                  ...
                  try {
                      $ibrQuery = "SELECT 
                                          ...
                                          CASE WHEN va.bh_ibr_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_ibr_history
                                      FROM 
                                          bufalino v
                                      LEFT JOIN 
                                          -- Join with all ibr records per tagid
                                          bh_ibr va ON v.tagid = va.bh_ibr_tagid 
                                      ORDER BY 
                                          va.bh_ibr_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($ibrsData)) {
                          ...
                          try {
                              $configQuery = "SELECT b_vencimiento_ibr FROM b_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['b_vencimiento_ibr'])) {
                                  $vigencia = intval($row_config['b_vencimiento_ibr']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($ibrsData as $row) {
                              ...
                              echo '        <button class="btn btn-success btn-sm register-new-ibr-btn" 
                              ...
                              if ($row['in_ibr_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-ibr" 
                                                  ...
                                                  data-vacuna="' . htmlspecialchars($row['bh_ibr_producto'] ?? '') . '" 
                              ...
                              if ($row['in_ibr_history'] == 1) {
                                  ...
                                  echo "<td>" . htmlspecialchars($row['bh_ibr_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['bh_ibr_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['bh_ibr_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['bh_ibr_fecha'])) {
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['bc_ibr_vacuna']) . '">' . htmlspecialchars($vacuna_row['bc_ibr_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
