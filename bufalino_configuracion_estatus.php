...
<title>Bufalino Configuracion Estatus</title>
<!-- Link to the Favicon -->
...
<div class="modal fade" id="newEntryModal" tabindex="-1" aria-labelledby="newEntryModalLabel">
  ...
  <!-- DataTable for bc_estatus records -->
...
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="estatusTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                      $estatusQuery = "SELECT * FROM bc_estatus";
                      ...
                      if (empty($estatussData)) {
                          ...
                          foreach ($estatussData as $row) {
                              ...
                              echo '        <button class="btn btn-danger btn-sm delete-estatus" 
                                              ...
                                              data-estatus="' . htmlspecialchars($row['bc_estatus_nombre'] ?? '') . '"
                              ...
                              // Column 1: Estatus
                              echo "<td>" . htmlspecialchars($row['bc_estatus_nombre'] ?? '') . "</td>";
...
<!-- Initialize DataTable for bufalino Estatus -->
<script>
                                ...
                                <div class="input-group">
                                    <span class="input-group-text">
                                        ...
                                        <select class="form-select" id="edit_estatus" name="estatus" required>
                                            ...
                                            $sql_names = "SELECT DISTINCT bc_estatus_nombre FROM bc_estatus ORDER BY bc_estatus_nombre ASC";
                                            $stmt_names = $conn->prepare($sql_names);
                                            $stmt_names->execute();
                                            $names = $stmt_names->fetchAll(PDO::FETCH_ASSOC);
                                            foreach ($names as $name_row) {
                                                echo '<option value="' . htmlspecialchars($name_row['bc_estatus_nombre']) . '">' . htmlspecialchars($name_row['bc_estatus_nombre']) . '</option>';
                                            }
                                            ?>
                                        </select>
                                    </span>                                    
