...
try {
    ...
    // Sal Expense per day = bh_sal_racion * bh_sal_costo
    ...
    $sql = "
        SELECT 
            ...
            bh_sal_costo
        FROM bh_sal 
        WHERE bh_sal_racion > 0 
          AND bh_sal_costo > 0 
          AND bh_sal_fecha_inicio IS NOT NULL
    ...
    if ($result) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            $startStr = $row['bh_sal_fecha_inicio'];
            $endStr = $row['bh_sal_fecha_fin'];
            $racion = (float)$row['bh_sal_racion'];
            $costo = (float)$row['bh_sal_costo'];
