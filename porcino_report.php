...
$sql_salt = "SELECT ph_sal_tagid, ph_sal_fecha, ph_sal_racion, ph_sal_costo FROM ph_sal WHERE ph_sal_tagid = ? ORDER BY ph_sal_fecha DESC";
...
if ($result_salt->num_rows > 0) {
    ...
    while ($row = $result_salt->fetch_assoc()) {
        $data[] = array($row['ph_sal_tagid'], $row['ph_sal_fecha'], $row['ph_sal_racion'], $row['ph_sal_costo']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_molasses = "SELECT ph_melaza_tagid, ph_melaza_fecha, ph_melaza_racion, ph_melaza_costo FROM ph_melaza WHERE ph_melaza_tagid = ? ORDER BY ph_melaza_fecha DESC";
...
if ($result_molasses->num_rows > 0) {
    ...
    while ($row = $result_molasses->fetch_assoc()) {
        $data[] = array($row['ph_melaza_tagid'], $row['ph_melaza_fecha'], $row['ph_melaza_racion'], $row['ph_melaza_costo']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_bruc = "SELECT ph_brucelosis_tagid, ph_brucelosis_fecha, ph_brucelosis_producto, ph_brucelosis_dosis FROM ph_brucelosis WHERE ph_brucelosis_tagid = ? ORDER BY ph_brucelosis_fecha DESC";
...
if ($result_bruc->num_rows > 0) {
    ...
    while ($row = $result_bruc->fetch_assoc()) {
        $data[] = array($row['ph_brucelosis_tagid'], $row['ph_brucelosis_fecha'], $row['ph_brucelosis_producto'], $row['ph_brucelosis_dosis']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_ibr = "SELECT ph_ibr_tagid, ph_ibr_fecha, ph_ibr_producto, ph_ibr_dosis FROM ph_ibr WHERE ph_ibr_tagid = ? ORDER BY ph_ibr_fecha DESC";
...
if ($result_ibr->num_rows > 0) {
    ...
    while ($row = $result_ibr->fetch_assoc()) {
        $data[] = array($row['ph_ibr_tagid'], $row['ph_ibr_fecha'], $row['ph_ibr_producto'], $row['ph_ibr_dosis']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_cbr = "SELECT ph_cbr_tagid, ph_cbr_fecha, ph_cbr_producto, ph_cbr_dosis FROM ph_cbr WHERE ph_cbr_tagid = ? ORDER BY ph_cbr_fecha DESC";
...
if ($result_cbr->num_rows > 0) {
    ...
    while ($row = $result_cbr->fetch_assoc()) {
        $data[] = array($row['ph_cbr_tagid'], $row['ph_cbr_fecha'], $row['ph_cbr_producto'], $row['ph_cbr_dosis']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_para = "SELECT ph_parasitos_tagid, ph_parasitos_fecha, ph_parasitos_producto, ph_parasitos_dosis FROM ph_parasitos WHERE ph_parasitos_tagid = ? ORDER BY ph_parasitos_fecha DESC";
...
$sql_tick = "SELECT ph_garrapatas_tagid, ph_garrapatas_fecha, ph_garrapatas_producto, ph_garrapatas_dosis FROM ph_garrapatas WHERE ph_garrapatas_tagid = ? ORDER BY ph_garrapatas_fecha DESC";
...
$sql_mastitis = "SELECT ph_mastitis_tagid, ph_mastitis_fecha, ph_mastitis_producto, ph_mastitis_dosis FROM ph_mastitis WHERE ph_mastitis_tagid = ? ORDER BY ph_mastitis_fecha DESC";
...
if ($result_mastitis->num_rows > 0) {
    ...
    while ($row = $result_mastitis->fetch_assoc()) {
        $data[] = array($row['ph_mastitis_tagid'], $row['ph_mastitis_fecha'], $row['ph_mastitis_producto'], $row['ph_mastitis_dosis']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_ins = "SELECT ph_inseminacion_tagid, ph_inseminacion_fecha, ph_inseminacion_numero FROM ph_inseminacion WHERE ph_inseminacion_tagid = ? ORDER BY ph_inseminacion_fecha DESC";
...
if ($result_ins->num_rows > 0) {
    ...
    while ($row = $result_ins->fetch_assoc()) {
        $data[] = array($row['ph_inseminacion_tagid'], $row['ph_inseminacion_fecha'], $row['ph_inseminacion_numero']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_preg = "SELECT ph_gestacion_tagid, ph_gestacion_fecha, ph_gestacion_numero FROM ph_gestacion WHERE ph_gestacion_tagid = ? ORDER BY ph_gestacion_fecha DESC";
...
if ($result_preg->num_rows > 0) {
    ...
    while ($row = $result_preg->fetch_assoc()) {
        $data[] = array($row['ph_gestacion_tagid'], $row['ph_gestacion_fecha'], $row['ph_gestacion_numero']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_birth = "SELECT ph_parto_tagid, ph_parto_fecha, ph_parto_numero FROM ph_parto WHERE ph_parto_tagid = ? ORDER BY ph_parto_fecha DESC";
...
if ($result_birth->num_rows > 0) {
    ...
    while ($row = $result_birth->fetch_assoc()) {
        $data[] = array($row['ph_parto_tagid'], $row['ph_parto_fecha'], $row['ph_parto_numero']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_breeds = "SELECT 
    ...
    ROUND((COUNT(*) * 100.0) / (SELECT COUNT(*) FROM porcino WHERE estatus = 'Activo'), 1) as porcentaje
FROM porcino 
...
$sql_monthly = "WITH MonthlyWeights AS (
    SELECT 
        DATE_FORMAT(ph_peso_fecha, '%Y-%m-01') as primer_dia_mes,
        ph_peso_tagid,
        AVG(ph_peso_animal) as peso_promedio_animal
    FROM ph_peso 
    GROUP BY DATE_FORMAT(ph_peso_fecha, '%Y-%m-01'), ph_peso_tagid
)
SELECT 
    primer_dia_mes as mes,
    COUNT(DISTINCT ph_peso_tagid) as total_animales,
...
$sql_feed_monthly = "WITH MonthlyFeed AS (
    SELECT 
        DATE_FORMAT(ph_concentrado_fecha_inicio, '%Y-%m-01') as primer_dia_mes,
        ph_concentrado_tagid,
        AVG(ph_concentrado_racion) as consumo_diario_promedio,
        AVG(ph_concentrado_costo) as costo_promedio
    FROM ph_concentrado
    GROUP BY DATE_FORMAT(ph_concentrado_fecha_inicio, '%Y-%m-01'), ph_concentrado_tagid
),
MonthlyStats AS (
    SELECT 
        primer_dia_mes as mes,
        COUNT(DISTINCT ph_concentrado_tagid) as total_animales,
...
MonthlyWeights AS (
    SELECT 
        ph_peso_tagid,
        DATE_FORMAT(ph_peso_fecha, '%Y-%m-01') as mes,
        AVG(ph_peso_animal) as peso_promedio
    FROM ph_peso
    GROUP BY ph_peso_tagid, DATE_FORMAT(ph_peso_fecha, '%Y-%m-01')
),
WeightChanges AS (
    SELECT 
        w1.ph_peso_tagid,
    ...
    JOIN MonthlyWeights w2 ON w1.ph_peso_tagid = w2.ph_peso_tagid
        AND w1.mes < w2.mes
        AND NOT EXISTS (
            SELECT 1 FROM MonthlyWeights w3
            WHERE w3.ph_peso_tagid = w1.ph_peso_tagid
...
TotalFeed AS (
    SELECT 
        ph_concentrado_tagid,
        DATE_FORMAT(ph_concentrado_fecha_inicio, '%Y-%m-01') as mes,
        SUM(ph_concentrado_racion) as consumo_total
    FROM ph_concentrado
    GROUP BY ph_concentrado_tagid, DATE_FORMAT(ph_concentrado_fecha_inicio, '%Y-%m-01')
),
FCRCalculation AS (
    SELECT 
        wc.ph_peso_tagid,
    ...
    JOIN AllAnimals a ON wc.ph_peso_tagid = a.tagid
    LEFT JOIN TotalFeed tf ON wc.ph_peso_tagid = tf.ph_concentrado_tagid
        AND tf.mes >= wc.mes_inicial AND tf.mes <= wc.mes_final
    GROUP BY wc.ph_peso_tagid, a.nombre, a.genero, a.etapa, wc.mes_inicial, wc.mes_final, 
...
SELECT 
    ...
    COUNT(DISTINCT ph_peso_tagid) as animales_con_ica,
...
$sql_molasses_monthly = "WITH MonthlyMolasses AS (
    SELECT 
        DATE_FORMAT(ph_melaza_fecha, '%Y-%m-01') as primer_dia_mes,
        ph_melaza_tagid,
        AVG(ph_melaza_racion) as consumo_diario_promedio,
        AVG(ph_melaza_costo) as costo_promedio
    FROM ph_melaza
    GROUP BY DATE_FORMAT(ph_melaza_fecha, '%Y-%m-01'), ph_melaza_tagid
),
MonthlyStats AS (
    SELECT 
        primer_dia_mes as mes,
        COUNT(DISTINCT ph_melaza_tagid) as total_animales,
        ROUND(SUM(consumo_diario_promedio), 2) as consumo_diario_total,
        ROUND(AVG(consumo_diario_promedio), 2) as promedio_diario_por_animal,
        ROUND(AVG(costo_promedio), 2) as costo_promedio_mes
...
$sql_salt_monthly = "WITH MonthlySalt AS (
    SELECT 
        DATE_FORMAT(ph_sal_fecha, '%Y-%m-01') as primer_dia_mes,
        ph_sal_tagid,
        AVG(ph_sal_racion) as consumo_diario_promedio,
        AVG(ph_sal_costo) as costo_promedio
    FROM ph_sal
    GROUP BY DATE_FORMAT(ph_sal_fecha, '%Y-%m-01'), ph_sal_tagid
),
MonthlyStats AS (
    SELECT 
        primer_dia_mes as mes,
        COUNT(DISTINCT ph_sal_tagid) as total_animales,
        ROUND(SUM(consumo_diario_promedio), 2) as consumo_diario_total,
        ROUND(AVG(consumo_diario_promedio), 2) as promedio_diario_por_animal,
        ROUND(AVG(costo_promedio), 2) as costo_promedio_mes
    FROM MonthlySalt
...
$sql_vacc_summary = "
WITH AllAnimals AS (
    SELECT DISTINCT tagid FROM porcino WHERE estatus = 'Activo'
),
VaccinationCounts AS (
    SELECT 
        (SELECT COUNT(*) FROM AllAnimals) as total_animals,
        (SELECT COUNT(DISTINCT ph_aftosa_tagid) FROM ph_aftosa) as aftosa_count,
        (SELECT COUNT(DISTINCT ph_brucelosis_tagid) FROM ph_brucelosis) as brucelosis_count,
        (SELECT COUNT(DISTINCT ph_cbr_tagid) FROM ph_cbr) as cbr_count,
        (SELECT COUNT(DISTINCT ph_ibr_tagid) FROM ph_ibr) as ibr_count,
        (SELECT COUNT(DISTINCT ph_carbunco_tagid) FROM ph_carbunco) as carbunco_count,
        (SELECT COUNT(DISTINCT ph_garrapatas_tagid) FROM ph_garrapatas) as garrapatas_count,
        (SELECT COUNT(DISTINCT ph_parasitos_tagid) FROM ph_parasitos) as parasitos_count,
        (SELECT COALESCE(SUM(ph_aftosa_costo * ph_aftosa_dosis), 0) FROM ph_aftosa) as aftosa_cost,
        (SELECT COALESCE(SUM(ph_brucelosis_costo * ph_brucelosis_dosis), 0) FROM ph_brucelosis) as brucelosis_cost,
        (SELECT COALESCE(SUM(ph_cbr_costo * ph_cbr_dosis), 0) FROM ph_cbr) as cbr_cost,
        (SELECT COALESCE(SUM(ph_ibr_costo * ph_ibr_dosis), 0) FROM ph_ibr) as ibr_cost,
        (SELECT COALESCE(SUM(ph_carbunco_costo * ph_carbunco_dosis), 0) FROM ph_carbunco) as carbunco_cost,
        (SELECT COALESCE(SUM(ph_garrapatas_costo * ph_garrapatas_dosis), 0) FROM ph_garrapatas) as garrapatas_cost,
        (SELECT COALESCE(SUM(ph_parasitos_costo * ph_parasitos_dosis), 0) FROM ph_parasitos) as parasitos_cost
)
SELECT 
    total_animals,
    aftosa_count,
...
$sql_preg_duration = "SELECT 
    g.ph_gestacion_tagid,
    v.nombre,
    g.ph_gestacion_numero,
    g.ph_gestacion_fecha,
    p.ph_parto_fecha,
    CASE 
        WHEN p.ph_parto_fecha IS NOT NULL THEN 'Completada'
        ELSE 'En Curso'
    END as estado,
    DATEDIFF(COALESCE(p.ph_parto_fecha, CURDATE()), g.ph_gestacion_fecha) as dias_gestacion
FROM ph_gestacion g
LEFT JOIN ph_parto p ON g.ph_gestacion_tagid = p.ph_parto_tagid 
    AND g.ph_gestacion_numero = p.ph_parto_numero
LEFT JOIN porcino v ON g.ph_gestacion_tagid = v.tagid
ORDER BY g.ph_gestacion_tagid, g.ph_gestacion_fecha DESC";
...
if ($result_preg_duration->num_rows > 0) {
    ...
    while ($row = $result_preg_duration->fetch_assoc()) {
        $parto_fecha = $row['ph_parto_fecha'] ? $row['ph_parto_fecha'] : 'En Curso';
        $data[] = array(
            $row['ph_gestacion_tagid'],
            $row['nombre'],
            $row['ph_gestacion_numero'],
            $row['ph_gestacion_fecha'],
            $parto_fecha,
            $row['estado'],
            $row['dias_gestacion']
        );
...
$sql_open_days = "WITH OrderedEvents AS (
    SELECT 
        ph_parto_tagid as tagid,
        ph_parto_fecha as fecha,
        ph_parto_numero as numero,
        'Parto' as tipo
    FROM ph_parto
    ...
    SELECT 
        ph_gestacion_tagid as tagid,
        ph_gestacion_fecha as fecha,
        ph_gestacion_numero as numero,
        'Gestacion' as tipo
    FROM ph_gestacion
),
NextPregnancy AS (
...
LEFT JOIN porcino v ON np.tagid = v.tagid
...
FROM porcino v
LEFT JOIN ph_gestacion g ON v.tagid = g.ph_gestacion_tagid
WHERE v.genero = 'Hembra' 
    AND g.ph_gestacion_tagid IS NULL
...
$sql_extended_period = "WITH LastBirth AS (
    SELECT 
        ph_parto_tagid,
        MAX(ph_parto_fecha) as ph_parto_fecha,
        COUNT(*) as total_partos
    FROM ph_parto
    GROUP BY ph_parto_tagid
)
SELECT 
    v.tagid,
    v.nombre,
    v.etapa,
    lb.ph_parto_fecha,
    DATEDIFF(CURDATE(), lb.ph_parto_fecha) as dias_desde_parto,
    lb.total_partos
FROM porcino v
JOIN LastBirth lb ON v.tagid = lb.ph_parto_tagid
LEFT JOIN ph_parto p ON v.tagid = p.ph_parto_tagid 
    AND p.ph_parto_fecha > lb.ph_parto_fecha
WHERE v.genero = 'Hembra' 
    AND v.estatus = 'Activo'
    AND DATEDIFF(CURDATE(), lb.ph_parto_fecha) > 365
    AND p.ph_parto_tagid IS NULL
