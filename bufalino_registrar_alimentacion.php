...
<div class="container scroll-icons-container">
    ...
    <button class="btn btn-outline-secondary mb-3" onclick="window.location.href='./bufalino_registrar_vacunacion.php'" data-tooltip="Salud">
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM bh_concentrado WHERE bh_concentrado_tagid = ? ORDER BY bh_concentrado_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM bh_concentrado ORDER BY bh_concentrado_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($concentradoData as $row) {
        $date = new DateTime($row['bh_concentrado_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['bh_concentrado_racion']);
        $concentradoFechaLabels[] = $row['bh_concentrado_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['bh_concentrado_costo']);
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['bh_concentrado_racion']) * floatval($row['bh_concentrado_costo']);
...
<?php
    ...
    $concentradoQuery = "SELECT p.*, 
                         ...
                         CAST((p.bh_concentrado_racion * p.bh_concentrado_costo) AS DECIMAL(10,2)) as total_value
                  FROM bh_concentrado p 
                  LEFT JOIN bufalino v ON p.bh_concentrado_tagid = v.tagid 
                  ORDER BY p.bh_concentrado_fecha DESC";
                ...
                .then(data => {
                    if (data.success) {
                        Swal.fire({
                            ...
                            window.location.href = data.redirect || 'bufalino_registrar_alimentacion.php';
                        });
                    } else {
...
foreach ($concentradoResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_concentrado_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_concentrado_producto']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_concentrado_etapa']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_concentrado_racion']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_concentrado_costo']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_concentrado_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-concentrado'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-producto='" . htmlspecialchars($row['bh_concentrado_producto']) . "'
                    data-etapa='" . htmlspecialchars($row['bh_concentrado_etapa']) . "'
                    data-racion='" . htmlspecialchars($row['bh_concentrado_racion']) . "'
                    data-costo='" . htmlspecialchars($row['bh_concentrado_costo']) . "'
                    data-fecha='" . htmlspecialchars($row['bh_concentrado_fecha']) . "'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM bh_melaza WHERE bh_melaza_tagid = ? ORDER BY bh_melaza_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM bh_melaza ORDER BY bh_melaza_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($melazaData as $row) {
        $date = new DateTime($row['bh_melaza_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['bh_melaza_racion']);
        $melazaFechaLabels[] = $row['bh_melaza_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['bh_melaza_costo']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['bh_melaza_racion']) * floatval($row['bh_melaza_costo']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $melazaQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         CAST((p.bh_melaza_racion * p.bh_melaza_costo) AS DECIMAL(10,2)) as total_value
                  FROM bh_melaza p 
                  LEFT JOIN bufalino v ON p.bh_melaza_tagid = v.tagid 
                  ORDER BY p.bh_melaza_fecha DESC";
...
foreach ($melazaResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_melaza_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_melaza_producto']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_melaza_etapa']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_melaza_racion']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_melaza_costo']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_melaza_fecha']) . "</td>";
    echo "<td class='text-center'>
            ...
            <button class='btn btn-success btn-sm edit-melaza'
                    ...
                    data-fecha='" . htmlspecialchars($row['bh_melaza_fecha']) . "'
                    data-bs-toggle='modal'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM bh_sal WHERE bh_sal_tagid = ? ORDER BY bh_sal_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM bh_sal ORDER BY bh_sal_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($salData as $row) {
        $date = new DateTime($row['bh_sal_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['bh_sal_racion']);
        $salFechaLabels[] = $row['bh_sal_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['bh_sal_costo']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['bh_sal_racion']) * floatval($row['bh_sal_costo']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $salQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         CAST((p.bh_sal_racion * p.bh_sal_costo) AS DECIMAL(10,2)) as total_value
                  FROM bh_sal p 
                  LEFT JOIN bufalino v ON p.bh_sal_tagid = v.tagid 
                  ORDER BY p.bh_sal_fecha DESC";
...
foreach ($salResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_sal_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_sal_producto']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_sal_etapa']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_sal_racion']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_sal_costo']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_sal_fecha']) . "</td>";
    echo "<td class='text-center'>
            ...
            <button class='btn btn-success btn-sm edit-sal'
                    ...
                    data-fecha='" . htmlspecialchars($row['bh_sal_fecha']) . "'
                    data-bs-toggle='modal'
