...
<title>Bufalino - Registros Parasitos</title>
<!-- Link to the Favicon -->
            ...
            <div class="row justify-content-center align-items-stretch">
                ...
                <div class="col-md-4 d-flex px-1 mb-3 mb-md-0">
                    <div class="arrow-step arrow-step-active w-100">
                        <span class="badge-active">🎯 Registrando Parasitos</span>
...
<div class="container text-center">
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT bc_parasitos_vacuna FROM bc_parasitos ORDER BY bc_parasitos_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['bc_parasitos_vacuna']) . '">' . htmlspecialchars($vacuna_row['bc_parasitos_vacuna']) . '</option>';
                                        }
  ...
  <!-- DataTable for bh_parasitos records -->
              ...
              <tbody>
                  ...
                  try {
                      $parasitosQuery = "SELECT 
                                          ...
                                          v.tagid,
                                          v.nombre,
                                          va.bh_parasitos_fecha,
                                          va.id AS parasitos_record_id, -- Alias parasitos record ID
                                          va.bh_parasitos_producto,
                                          va.bh_parasitos_dosis,
                                          va.bh_parasitos_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.bh_parasitos_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_parasitos_history
                                      FROM 
                                          bufalino v
                                      LEFT JOIN 
                                          -- Join with all parasitos records per tagid
                                          bh_parasitos va ON v.tagid = va.bh_parasitos_tagid
                                      ORDER BY 
                                          va.bh_parasitos_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($parasitossData)) {
                          ...
                          try {
                              $configQuery = "SELECT b_vencimiento_parasitos FROM b_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['b_vencimiento_parasitos'])) {
                                  $vigencia = intval($row_config['b_vencimiento_parasitos']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($parasitossData as $row) {
                              ...
                              if ($row['in_parasitos_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-parasitos" 
                                                  ...
                                                  data-vacuna="' . htmlspecialchars($row['bh_parasitos_producto'] ?? '') . '" 
                                                  data-dosis="' . htmlspecialchars($row['bh_parasitos_dosis'] ?? '') . '" 
                                                  data-costo="' . htmlspecialchars($row['bh_parasitos_costo'] ?? '') . '" 
                                                  data-fecha="' . htmlspecialchars($row['bh_parasitos_fecha'] ?? '') . '"
                                                  title="Editar Vacuna">
                              ...
                              echo "<td>" . htmlspecialchars($row['bh_parasitos_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_parasitos_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['bh_parasitos_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['bh_parasitos_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['bh_parasitos_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['bh_parasitos_fecha'])) {
                                          $parasitosDate = new DateTime($row['bh_parasitos_fecha']);
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['bh_parasitos_fecha']);
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['bc_parasitos_vacuna']) . '">' . htmlspecialchars($vacuna_row['bc_parasitos_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
