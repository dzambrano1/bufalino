...
try {
    ...
    $sql = "
        SELECT
            ...
            bh_melaza_costo
        FROM bh_melaza
        WHERE bh_melaza_racion > 0 AND bh_melaza_costo > 0 -- Consider only valid cost entries
        ORDER BY bh_melaza_tagid, bh_melaza_fecha ASC;
    ";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            $tagid = $row['bh_melaza_tagid'];
            $fecha = $row['bh_melaza_fecha'];
            $daily_cost = (float)$row['bh_melaza_racion'] * (float)$row['bh_melaza_costo'];
