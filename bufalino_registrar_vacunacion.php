<?php
require_once './pdo_conexion.php';  // Go up one directory since inventario_bufalino.php is in the bufalino folder
...
<div class="container" id="nav-buttons">
    <div class="container nav-icons-container" id="nav-buttons">
        ...
        <button onclick="window.location.href='./inventario_bufalino.php'" class="icon-button" data-tooltip="Inventario bufalino">
            <img src="./images/bufalo-icon.png" alt="Inventario" class="nav-icon">
        </button>
        
        <button onclick="window.location.href='./bufalino_indices.php'" class="icon-button" data-tooltip="Indices bufalinos">
...
<div class="container scroll-icons-container">    
    <button class="btn btn-outline-secondary mb-3" onclick="window.location.href='./bufalino_registrar_pesaje_animal.php'" data-tooltip="Pesaje Carne">
        <img src="./images/carne.png" alt="Pesaje Carne" class="nav-icon">
    </button>
    
    <button class="btn btn-outline-secondary mb-3" onclick="window.location.href='./bufalino_registrar_pesaje_leche.php'" data-tooltip="Pesaje Leche">
        <img src="./images/leche-cantaro.png" alt="Pesaje Leche" class="nav-icon">
    </button>
    
    <button class="btn btn-outline-secondary mb-3" onclick="window.location.href='./bufalino_registrar_alimentacion.php'" data-tooltip="Alimentacion">
    ...
    <button  class="btn btn-outline-secondary mb-3" type="button" onclick="window.location.href='./bufalino_registrar_reproduccion.php'" data-tooltip="Reproduccion">
        <img src="./images/el-embarazo.png" alt="Reproduccion" class="nav-icon">
    </button>

    <button class="btn btn-outline-secondary mb-3" onclick="window.location.href='./bufalino_registrar_poblacion.php'" data-tooltip="Poblacion">
        <img src="./images/bufalo-icon.png" alt="Poblacion" class="nav-icon">
    </button>
</div>
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM bh_aftosa WHERE bh_aftosa_tagid = ? ORDER BY bh_aftosa_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM bh_aftosa ORDER BY bh_aftosa_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($aftosaData as $row) {
        $date = new DateTime($row['bh_aftosa_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['bh_aftosa_dosis']);
        $aftosaFechaLabels[] = $row['bh_aftosa_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['bh_aftosa_costo']);
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['bh_aftosa_dosis']) * floatval($row['bh_aftosa_costo']);
...
<?php
    ...
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $tagid = $_GET['search'];
        $stmt = $conn->prepare("SELECT nombre FROM bufalino WHERE tagid = ?");
        $stmt->execute([$tagid]);
        if ($row = $stmt->fetch()) {
    ...
    $aftosaQuery = "SELECT p.*, 
                         ...
                         CAST((p.bh_aftosa_dosis * p.bh_aftosa_costo) AS DECIMAL(10,2)) as total_value
                  FROM bh_aftosa p 
                  LEFT JOIN bufalino v ON p.bh_aftosa_tagid = v.tagid 
                  ORDER BY p.bh_aftosa_fecha DESC";
                ...
                .then(data => {
                    if (data.success) {
                        Swal.fire({
                            ...
                            window.location.href = data.redirect || 'bufalino_registrar_vacunacion.php';
                        });
                    } else {
...
foreach ($aftosaResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_aftosa_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_aftosa_producto']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_aftosa_dosis']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_aftosa_costo']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_aftosa_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-aftosa'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-producto='" . htmlspecialchars($row['bh_aftosa_producto']) . "'
                    data-dosis='" . htmlspecialchars($row['bh_aftosa_dosis']) . "'
                    data-costo='" . htmlspecialchars($row['bh_aftosa_costo']) . "'
                    data-fecha='" . htmlspecialchars($row['bh_aftosa_fecha']) . "'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM bh_ibr WHERE bh_ibr_tagid = ? ORDER BY bh_ibr_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM bh_ibr ORDER BY bh_ibr_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($ibrData as $row) {
        $date = new DateTime($row['bh_ibr_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['bh_ibr_dosis']);
        $ibrFechaLabels[] = $row['bh_ibr_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['bh_ibr_costo']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['bh_ibr_dosis']) * floatval($row['bh_ibr_costo']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $ibrQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         CAST((p.bh_ibr_dosis * p.bh_ibr_costo) AS DECIMAL(10,2)) as total_value
                  FROM bh_ibr p 
                  LEFT JOIN bufalino v ON p.bh_ibr_tagid = v.tagid 
                  ORDER BY p.bh_ibr_fecha DESC";
...
foreach ($ibrResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_ibr_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_ibr_producto']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_ibr_dosis']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_ibr_costo']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_ibr_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-ibr'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-producto='" . htmlspecialchars($row['bh_ibr_producto']) . "'                    
                    data-dosis='" . htmlspecialchars($row['bh_ibr_dosis']) . "'
                    data-costo='" . htmlspecialchars($row['bh_ibr_costo']) . "'
                    data-fecha='" . htmlspecialchars($row['bh_ibr_fecha']) . "'
                    data-bs-toggle='modal'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM bh_cbr WHERE bh_cbr_tagid = ? ORDER BY bh_cbr_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM bh_cbr ORDER BY bh_cbr_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($cbrData as $row) {
        $date = new DateTime($row['bh_cbr_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['bh_cbr_dosis']);
        $cbrFechaLabels[] = $row['bh_cbr_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['bh_cbr_costo']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['bh_cbr_dosis']) * floatval($row['bh_cbr_costo']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $cbrQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         CAST((p.bh_cbr_dosis * p.bh_cbr_costo) AS DECIMAL(10,2)) as total_value
                  FROM bh_cbr p 
                  LEFT JOIN bufalino v ON p.bh_cbr_tagid = v.tagid 
                  ORDER BY p.bh_cbr_fecha DESC";
...
foreach ($cbrResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_cbr_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_cbr_producto']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_cbr_dosis']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_cbr_costo']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_cbr_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-cbr'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-producto='" . htmlspecialchars($row['bh_cbr_producto']) . "'
                    data-dosis='" . htmlspecialchars($row['bh_cbr_dosis']) . "'
                    data-costo='" . htmlspecialchars($row['bh_cbr_costo']) . "'
                    data-fecha='" . htmlspecialchars($row['bh_cbr_fecha']) . "'
                    data-bs-toggle='modal'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM bh_brucelosis WHERE bh_brucelosis_tagid = ? ORDER BY bh_brucelosis_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM bh_brucelosis ORDER BY bh_brucelosis_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($brucelosisData as $row) {
        $date = new DateTime($row['bh_brucelosis_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['bh_brucelosis_dosis']);
        $brucelosisFechaLabels[] = $row['bh_brucelosis_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['bh_brucelosis_costo']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['bh_brucelosis_dosis']) * floatval($row['bh_brucelosis_costo']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $brucelosisQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         CAST((p.bh_brucelosis_dosis * p.bh_brucelosis_costo) AS DECIMAL(10,2)) as total_value
                  FROM bh_brucelosis p 
                  LEFT JOIN bufalino v ON p.bh_brucelosis_tagid = v.tagid 
                  ORDER BY p.bh_brucelosis_fecha DESC";
...
foreach ($brucelosisResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_brucelosis_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_brucelosis_producto']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_brucelosis_dosis']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_brucelosis_costo']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_brucelosis_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-brucelosis'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-producto='" . htmlspecialchars($row['bh_brucelosis_producto']) . "'
                    data-dosis='" . htmlspecialchars($row['bh_brucelosis_dosis']) . "'
                    data-costo='" . htmlspecialchars($row['bh_brucelosis_costo']) . "'
                    data-fecha='" . htmlspecialchars($row['bh_brucelosis_fecha']) . "'
                    data-bs-toggle='modal'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM bh_carbunco WHERE bh_carbunco_tagid = ? ORDER BY bh_carbunco_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM bh_carbunco ORDER BY bh_carbunco_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($carbuncoData as $row) {
        $date = new DateTime($row['bh_carbunco_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['bh_carbunco_dosis']);
        $carbuncoFechaLabels[] = $row['bh_carbunco_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['bh_carbunco_costo']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['bh_carbunco_dosis']) * floatval($row['bh_carbunco_costo']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $carbuncoQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         CAST((p.bh_carbunco_dosis * p.bh_carbunco_costo) AS DECIMAL(10,2)) as total_value
                  FROM bh_carbunco p 
                  LEFT JOIN bufalino v ON p.bh_carbunco_tagid = v.tagid 
                  ORDER BY p.bh_carbunco_fecha DESC";
...
foreach ($carbuncoResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_carbunco_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_carbunco_producto']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_carbunco_dosis']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_carbunco_costo']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_carbunco_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-carbunco'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-producto='" . htmlspecialchars($row['bh_carbunco_producto']) . "'
                    data-dosis='" . htmlspecialchars($row['bh_carbunco_dosis']) . "'
                    data-costo='" . htmlspecialchars($row['bh_carbunco_costo']) . "'
                    data-fecha='" . htmlspecialchars($row['bh_carbunco_fecha']) . "'
                    data-bs-toggle='modal'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM bh_garrapatas WHERE bh_garrapatas_tagid = ? ORDER BY bh_garrapatas_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM bh_garrapatas ORDER BY bh_garrapatas_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($garrapatasData as $row) {
        $date = new DateTime($row['bh_garrapatas_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['bh_garrapatas_dosis']);
        $garrapatasFechaLabels[] = $row['bh_garrapatas_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['bh_garrapatas_costo']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['bh_garrapatas_dosis']) * floatval($row['bh_garrapatas_costo']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $garrapatasQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         CAST((p.bh_garrapatas_dosis * p.bh_garrapatas_costo) AS DECIMAL(10,2)) as total_value
                  FROM bh_garrapatas p 
                  LEFT JOIN bufalino v ON p.bh_garrapatas_tagid = v.tagid 
                  ORDER BY p.bh_garrapatas_fecha DESC";
...
foreach ($garrapatasResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_garrapatas_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_garrapatas_producto']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_garrapatas_dosis']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_garrapatas_costo']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_garrapatas_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-garrapatas'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-producto='" . htmlspecialchars($row['bh_garrapatas_producto']) . "'
                    data-dosis='" . htmlspecialchars($row['bh_garrapatas_dosis']) . "'
                    data-costo='" . htmlspecialchars($row['bh_garrapatas_costo']) . "'
                    data-fecha='" . htmlspecialchars($row['bh_garrapatas_fecha']) . "'
                    data-bs-toggle='modal'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM bh_lombrices WHERE bh_lombrices_tagid = ? ORDER BY bh_lombrices_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM bh_lombrices ORDER BY bh_lombrices_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($lombricesData as $row) {
        $date = new DateTime($row['bh_lombrices_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['bh_lombrices_dosis']);
        $lombricesFechaLabels[] = $row['bh_lombrices_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['bh_lombrices_costo']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['bh_lombrices_dosis']) * floatval($row['bh_lombrices_costo']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $lombricesQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         CAST((p.bh_lombrices_dosis * p.bh_lombrices_costo) AS DECIMAL(10,2)) as total_value
                  FROM bh_lombrices p 
                  LEFT JOIN bufalino v ON p.bh_lombrices_tagid = v.tagid 
                  ORDER BY p.bh_lombrices_fecha DESC";
...
foreach ($lombricesResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_lombrices_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_lombrices_producto']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_lombrices_dosis']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_lombrices_costo']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_lombrices_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-lombrices'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-producto='" . htmlspecialchars($row['bh_lombrices_producto']) . "'
                    data-dosis='" . htmlspecialchars($row['bh_lombrices_dosis']) . "'
                    data-costo='" . htmlspecialchars($row['bh_lombrices_costo']) . "'
                    data-fecha='" . htmlspecialchars($row['bh_lombrices_fecha']) . "'
                    data-bs-toggle='modal'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM bh_mastitis WHERE bh_mastitis_tagid = ? ORDER BY bh_mastitis_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM bh_mastitis ORDER BY bh_mastitis_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($mastitisData as $row) {
        $date = new DateTime($row['bh_mastitis_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['bh_mastitis_dosis']);
        $mastitisFechaLabels[] = $row['bh_mastitis_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['bh_mastitis_costo']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['bh_mastitis_dosis']) * floatval($row['bh_mastitis_costo']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $mastitisQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         CAST((p.bh_mastitis_dosis * p.bh_mastitis_costo) AS DECIMAL(10,2)) as total_value
                  FROM bh_mastitis p 
                  LEFT JOIN bufalino v ON p.bh_mastitis_tagid = v.tagid 
                  ORDER BY p.bh_mastitis_fecha DESC";
...
foreach ($mastitisResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_mastitis_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_mastitis_producto']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_mastitis_dosis']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_mastitis_costo']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['bh_mastitis_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-mastitis'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-producto='" . htmlspecialchars($row['bh_mastitis_producto']) . "'
                    data-dosis='" . htmlspecialchars($row['bh_mastitis_dosis']) . "'
                    data-costo='" . htmlspecialchars($row['bh_mastitis_costo']) . "'
                    data-fecha='" . htmlspecialchars($row['bh_mastitis_fecha']) . "'
                    data-bs-toggle='modal'
