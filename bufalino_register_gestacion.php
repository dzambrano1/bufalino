...
$chartData = [];
try {
    $chartQuery = "SELECT DATE_FORMAT(bh_gestacion_fecha, '%Y-%m') as month_year, COUNT(*) as count 
                     FROM bh_gestacion 
                     GROUP BY month_year 
...
<title>Bufalino Register Gestacion</title>
<!-- Link to the Favicon -->
            ...
            <div class="row justify-content-center align-items-stretch">
                ...
                <div class="col-md-4 d-flex px-1 mb-3 mb-md-0">
                    <div class="arrow-step arrow-step-active w-100">
                        <span class="badge-active">🎯 Registrando Gestacion</span>
              ...
              <?php
                  try {
                      // Query to get all Female animals and their LATEST gestacion record (if any)
                        $gestacionQuery = "
                            SELECT
                                ...
                                ins.bh_gestacion_fecha,
                                ins.bh_gestacion_tagid, -- Will be NULL if no gestacion
                                ins.bh_gestacion_numero
                            FROM
                                bufalino v
                            LEFT JOIN bh_gestacion ins
                              ON v.tagid = ins.bh_gestacion_tagid
                            LEFT JOIN bh_gestacion ins_check
                              ON ins.bh_gestacion_tagid = ins_check.bh_gestacion_tagid
                             AND ins.bh_gestacion_fecha < ins_check.bh_gestacion_fecha -- Check for a newer record
                            ...
                            ORDER BY
                                ...
                                ins.bh_gestacion_fecha DESC, -- Then by latest insemination date
                      ...
                      if (empty($gestacionData)) {
                          ...
                          try {
                              $configQuery = "SELECT b_vencimiento_gestacion FROM b_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              
                              // Explicitly use PDO fetch method
                              $row = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row && isset($row['b_vencimiento_gestacion'])) {
                                  $vigencia = intval($row['b_vencimiento_gestacion']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($gestacionData as $row) {
                              ...
                              $gestacionFecha = $row['bh_gestacion_fecha'] ?? null;
                              ...
                              echo "<td class='text-center'>" . ($hasgestacion ? htmlspecialchars($row['bh_gestacion_numero'] ?? '') : 'N/A') . "</td>";                              
