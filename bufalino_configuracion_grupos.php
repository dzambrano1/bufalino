...
<title>Bufalino Configuracion Grupos</title>
<!-- Link to the Favicon -->
...
<div class="modal fade" id="newEntryModal" tabindex="-1" aria-labelledby="newEntryModalLabel">
  ...
  <!-- DataTable for bc_grupos records -->
...
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="gruposTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                      $gruposQuery = "SELECT * FROM bc_grupos";
                      ...
                      if (empty($gruposData)) {
                          ...
                          foreach ($gruposData as $row) {
                              ...
                              echo '        <button class="btn btn-danger btn-sm delete-grupos" 
                                              ...
                                              data-grupos="' . htmlspecialchars($row['bc_grupos_nombre'] ?? '') . '"
                              ...
                              // Column 1: Grupos
                              echo "<td>" . htmlspecialchars($row['bc_grupos_nombre'] ?? '') . "</td>";
...
<!-- Initialize DataTable for bufalino Grupos -->
<script>
                                ...
                                <div class="input-group">
                                    <span class="input-group-text">
                                        ...
                                        <select class="form-select" id="edit_grupos" name="grupos" required>
                                            ...
                                            $sql_names = "SELECT DISTINCT bc_grupos_nombre FROM bc_grupos ORDER BY bc_grupos_nombre ASC";
                                            $stmt_names = $conn->prepare($sql_names);
                                            $stmt_names->execute();
                                            $names = $stmt_names->fetchAll(PDO::FETCH_ASSOC);
                                            foreach ($names as $name_row) {
                                                echo '<option value="' . htmlspecialchars($name_row['bc_grupos_nombre']) . '">' . htmlspecialchars($name_row['bc_grupos_nombre']) . '</option>';
                                            }
                                            ?>
                                        </select>
                                    </span>                                    
