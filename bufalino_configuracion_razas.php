...
<title>Bufalino Configuracion Razas</title>
<!-- Link to the Favicon -->
...
<div class="modal fade" id="newEntryModal" tabindex="-1" aria-labelledby="newEntryModalLabel">
  ...
  <!-- DataTable for bc_razas records -->
...
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="razasTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                      $razasQuery = "SELECT * FROM bc_razas";
                      ...
                      if (empty($razassData)) {
                          ...
                          foreach ($razassData as $row) {
                              ...
                              echo '        <button class="btn btn-danger btn-sm delete-razas" 
                                              ...
                                              data-razas="' . htmlspecialchars($row['bc_razas_nombre'] ?? '') . '"
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['bc_razas_nombre'] ?? '') . "</td>";
                                ...
                                <div class="input-group">
                                    <span class="input-group-text">
                                        ...
                                        <select class="form-select" id="edit_razas" name="razas" required>
                                            ...
                                            $sql_names = "SELECT DISTINCT bc_razas_nombre FROM bc_razas ORDER BY bc_razas_nombre ASC";
                                            $stmt_names = $conn->prepare($sql_names);
                                            $stmt_names->execute();
                                            $names = $stmt_names->fetchAll(PDO::FETCH_ASSOC);
                                            foreach ($names as $name_row) {
                                                echo '<option value="' . htmlspecialchars($name_row['bc_razas_nombre']) . '">' . htmlspecialchars($name_row['bc_razas_nombre']) . '</option>';
                                            }
                                            ?>
                                        </select>
                                    </span>                                    
