...
try {
    ...
    $melazaQuery = "SELECT 
                            bh_melaza_fecha_inicio,
                            bh_melaza_fecha_fin,
                            bh_melaza_racion,
                            bh_melaza_costo,
                            bh_melaza_tagid
                        FROM bh_melaza 
                        WHERE bh_melaza_fecha_inicio IS NOT NULL 
                        AND bh_melaza_fecha_fin IS NOT NULL 
                        AND bh_melaza_racion IS NOT NULL 
                        AND bh_melaza_costo IS NOT NULL
                        ORDER BY bh_melaza_fecha_inicio ASC";
    ...
    if (empty($melazaData)) {
        ...
        $melazaQuery = "SELECT 
                                bh_melaza_fecha,
                                bh_melaza_fecha as bh_melaza_fecha_inicio,
                                bh_melaza_fecha as bh_melaza_fecha_fin,
                                bh_melaza_racion,
                                bh_melaza_costo,
                                bh_melaza_tagid
                            FROM bh_melaza 
                            WHERE bh_melaza_fecha IS NOT NULL 
                            AND bh_melaza_racion IS NOT NULL 
                            AND bh_melaza_costo IS NOT NULL
    ...
    foreach ($melazaData as $melaza) {
        $startDate = new DateTime($melaza['bh_melaza_fecha_inicio']);
        $endDate = new DateTime($melaza['bh_melaza_fecha_fin']);
        
        // Calculate total expense value
        $totalDays = $endDate->diff($startDate)->days + 1;
        $totalValue = $melaza['bh_melaza_racion'] * $melaza['bh_melaza_costo'] * $totalDays;
        $dailyValue = $totalValue / $totalDays;
        
        error_log("Processing melaza: Start=" . $melaza['bh_melaza_fecha_inicio'] . 
                 ", End=" . $melaza['bh_melaza_fecha_fin'] . 
                 ", Days=" . $totalDays . 
                  ...
                  try {
                      // Query to get all Animals and ALL their melaza records (if any)
                        $melazaQuery = "
                            SELECT
                                ...
                                c.bh_melaza_fecha_inicio,
                                c.bh_melaza_fecha_fin,
                            ...
                            FROM
                                bufalino v
                            LEFT JOIN
                                bh_melaza c ON v.tagid = c.bh_melaza_tagid -- Join ALL matching melaza records
                            ORDER BY
                      ...
                      if (empty($melazaData)) {
                          ...
                          foreach ($melazaData as $row) {
                              ...
                              $melazaFecha = $row['bh_melaza_fecha_inicio'] ?? null;
                              $melazaFechaInicio = $row['bh_melaza_fecha_inicio'] ?? null;
                              $melazaFechaFin = $row['bh_melaza_fecha_fin'] ?? null;
                              ...
                              if ($hasMelaza) {
                                  ...
                                  echo '        <button class="btn btn-warning btn-sm edit-melaza" 
                                                  ...
                                                  data-debug="DB: '.htmlspecialchars($row['bh_melaza_producto'] ?? 'NULL').'">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="productList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $alimentos = $stmt_alimentos->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($alimentos as $alimento_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action product-option" data-value="' . htmlspecialchars($alimento_row['bc_melaza_nombre']) . '">' . htmlspecialchars($alimento_row['bc_melaza_nombre']) . '</button>';
                                    }
                                } catch (PDOException $e) {
