...
try {
    ...
    $sql_groups = "SELECT DISTINCT grupo FROM bufalino WHERE grupo IS NOT NULL AND grupo != '' ORDER BY grupo";
    ...
    $sql_counts = "
        SELECT
            ...
            COUNT(DISTINCT a.bh_brucelosis_tagid) AS vaccinated_animals
        FROM bufalino v
        LEFT JOIN bh_brucelosis a ON v.tagid = a.bh_brucelosis_tagid
    ...
    $sql_list = "
        ...
        LEFT JOIN bh_brucelosis a ON v.tagid = a.bh_brucelosis_tagid
        WHERE v.grupo IS NOT NULL AND v.grupo != ''
          AND a.bh_brucelosis_tagid IS NULL
